...
class MotionPlanner:
    def __init__(self, model, T=1, N=50, nlp_solver='ipopt'):
        ...
        self.r_final = np.array([0.15, 0.0, 0.68])
        self.p_left_init = np.array([0.0, 0.21, 0.0])
        self.p_right_init = np.array([0.0, -0.21, 0.0])
        self.p_left_final = np.array([0.15, 0.21, 0.1])
        self.p_right_final = np.array([0.15, -0.21, 0.1])
    ...
    def trajectory_generation(self, r, rd, p_left, p_right, dt, Hd):
        ...
        np.savez('data_partial_foothold.npz', copl=sample_lFOOT, copr=sample_rFOOT, time=sample_t, com=sample_COM, dcom=sample_dCOM, ddcom=sample_ddCOM, phase=sample_phase)
        plt.show()
