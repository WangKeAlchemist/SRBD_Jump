...
class SingleRigidBodyModel:
    def __init__(self, mass=13.8, size=(0.25, 0.4, 0.25), g=(0, 0, -9.81)):
        ...
        self.size = size
        self.g = np.array(g)
        self.I_body = self.local_inertia_tensor(mass, size)*1.5 # to match the real one
    ...
    def dynamics(self, r, q, H, L, f1, f2, f3, f4, f5, f6, f7, f8, p1, p2, p3, p4, p5, p6, p7, p8):
        ...
        I = R @ self.I_body @ R.T
        I_inv = R @ self.I_body_inv @ R.T
        omega = I_inv @ (L) # -self.mass*cross(r, rd)
        qd = 1 / 2 * self.Q(q) @ omega
        ...
        Ld = cross(f1, r - p1) + cross(f2, r - p2) + cross(f3, r - p3) + cross(f4, r - p4) + cross(f5, r - p5) + cross(
            f6, r - p6) + cross(f7, r - p7) + cross(f8, r - p8) #+ self.mass * cross(r, self.g)
        return rd, qd, Hd, Ld, omega
...
class MotionPlanner:
    def __init__(self, model, T=1, N=50, nlp_solver='ipopt'):
        ...
        # side jump: 1.05
        self.r_init = np.array([0.0, 0.0, 0.64])
        self.r_final = np.array([0.0, 0.0, 0.64])
        ...
        # self.p_left_final = np.array([0.21, 0.0, 0.0])
        # self.p_right_final = np.array([-0.21, 0.0, 0.0])
        # rotate by orientation
        # self.p_left_final = self.p_left_init
        # self.p_right_final = self.p_right_init
        self.p_left_final = R@self.p_left_init
        self.p_right_final = R@self.p_right_init
        # Optimization solver
        self.opti = Opti()
        ...
        self.omega = self.opti.variable(3, self.NX)
        ...
        p1_final = self.p_left_final + R @ np.array([-self.model.foot_length / 2, self.model.foot_width / 2, 0.0])
        p2_final = self.p_left_final + R @ np.array([self.model.foot_length / 2, self.model.foot_width / 2, 0.0])
        p3_final = self.p_left_final + R @ np.array([self.model.foot_length / 2, -self.model.foot_width / 2, 0.0])
        p4_final = self.p_left_final + R @ np.array([-self.model.foot_length / 2, -self.model.foot_width / 2, 0.0])
        p5_final = self.p_right_final + R @ np.array([-self.model.foot_length / 2, self.model.foot_width / 2, 0.0])
        p6_final = self.p_right_final + R @ np.array([self.model.foot_length / 2, self.model.foot_width / 2, 0.0])
        p7_final = self.p_right_final + R @ np.array([self.model.foot_length / 2, -self.model.foot_width / 2, 0.0])
        p8_final = self.p_right_final + R @ np.array([-self.model.foot_length / 2, -self.model.foot_width / 2, 0.0])
        ...
        q_fin_cost = sumsqr(self.q[:, -1] - self.q_final) #+ sumsqr(self.omega[:, -1])
        # here transpose is a MUST!!!
        effort_cost = sumsqr(self.Ld) + sumsqr(self.Hd) + sumsqr(diff(p_left.T)) + sumsqr(diff(p_right.T)) +sumsqr(diff(f1.T)) + sumsqr(diff(f2.T)) \
                     + sumsqr(diff(f3.T)) + sumsqr(diff(f4.T)) + sumsqr(diff(f5.T)) + sumsqr(diff(f6.T)) + sumsqr(diff(f7.T)) + sumsqr(diff(f8.T)) #+ sumsqr(self.L[:,-1]) + sumsqr(diff(self.L))#+ sumsqr(self.L[:, -5:])+ sumsqr(self.L[:, :5])
        costs = q_fin_cost + 1e-5*effort_cost
        ...
        self.opti.subject_to(self.omega[:, 0] == np.array([0, 0, 0]))
        self.opti.subject_to(self.r[:, -1] == self.r_final)
        # self.opti.subject_to(self.q[:, -1] >= tf.quaternion_from_euler(0.0,0.0,np.pi /2-0.2))
        # self.opti.subject_to(self.q[:, -1] <= tf.quaternion_from_euler(0.0,0.0,np.pi /2+0.2))
        self.opti.subject_to(self.H[:, -1] == np.array([0,0,0.0]))
        self.opti.subject_to(self.L[:, -1] == np.array([0.0,0.0,0.0]))
        # dynamic constraints
        for i in range(3):
            for j in range(self.N_array[i]):
                ...
                self.rd[:,k], self.qd[:,k], self.Hd[:,k], self.Ld[:,k], self.omega[:,k] = self.model.dynamics(self.r[:, k], self.q[:, k], self.H[:, k], self.L[:, k],
        ...
        for k in range(self.N):
            ...
            if k < self.N1:
                ...
                # single leg support
                self.opti.subject_to(0.40 < self.r[2,k] - self.p_left_init[2])
                ...
                # self.opti.subject_to(f7[2, k] >= 0)
                # self.opti.subject_to(f8[2, k] >= 0)
                self.opti.subject_to(0.4 < self.r[2,k] - p_left[2, k])
                self.opti.subject_to(self.r[2,k] - p_left[2, k] < 0.75)
                self.opti.subject_to(0.4 < self.r[2,k] - p_right[2, k])
                ...
                self.opti.subject_to(f7[2, k] >= 0)
                self.opti.subject_to(f8[2, k] >= 0)
                self.opti.subject_to(0.45 < self.r[2,k] - self.p_left_final[2])
        ...
        self.trajectory_generation(solution.value(self.r),solution.value(self.rd),solution.value(p_left), solution.value(p_right),
                                   solution.value(self.dt), solution.value(self.Hd), solution.value(self.L), solution.value(self.Ld),
                                   solution.value(self.q), solution.value(self.omega))

    def set_initial_solution(self):
        # probably need initialization of q, definitely
        q_slerp = np.array([tf.quaternion_slerp(self.q_init, self.q_final, i / (self.NX)) for i in range(self.NX)])
        # q_final_init = np.tile(self.q_final, (5,1))
        # q_slerp = np.concatenate((q_slerp, q_final_init))
        self.opti.set_initial(self.q, q_slerp.T)
    ...
    def euler_from_quaternion(self, q):
        length = q.shape[1]
        euler = np.zeros((3, length))
        for i in range(length):
            euler[0,i], euler[1,i], euler[2,i] = tf.euler_from_quaternion(q[:,i])
        return euler

    def trajectory_generation(self, r, rd, p_left, p_right, dt, Hd, L, Ld, q, omega):
        ...
        ori_foot_z = np.ones(self.NX)
        ori_foot_z[:self.N1] *= euler[2,0]
        for i in range(self.N2):
            ori_foot_z[self.N1+i] *= euler[2,0] + (euler[2,-1]-euler[2,0])*i/self.N2
        ori_foot_z[self.N1+self.N2:] *= euler[2,-1]
        ...
        Angx = interp1d(dt_plot, L[0,:], kind='linear')
        Angy = interp1d(dt_plot, L[1,:], kind='linear')
        Angz = interp1d(dt_plot, L[2,:], kind='linear')
        Angdx = interp1d(dt_plot, Ld[0,:], kind='quadratic')
        Angdy = interp1d(dt_plot, Ld[1,:], kind='quadratic')
        Angdz = interp1d(dt_plot, Ld[2,:], kind='quadratic')
        anglex = interp1d(dt_plot, euler[0,:], kind='linear')
        angley = interp1d(dt_plot, euler[1,:], kind='linear')
        anglez = interp1d(dt_plot, euler[2,:], kind='linear')
        angledx = interp1d(dt_plot, omega[0,:], kind='linear')
        angledy = interp1d(dt_plot, omega[1,:], kind='linear')
        angledz = interp1d(dt_plot, omega[2,:], kind='linear')
        ...
        sample_anglex = anglex(sample_t)
        sample_angley = angley(sample_t)
        sample_anglez = anglez(sample_t)
        sample_angledx = angledx(sample_t)
        sample_angledy = angledy(sample_t)
        sample_angledz = angledz(sample_t)
        ...
        sample_q = np.vstack((sample_anglex, sample_angley, sample_anglez))
        sample_dq = np.vstack((sample_angledx, sample_angledy, sample_angledz))
        ...
        np.savez('data_spin_z90.npz', copl=sample_lFOOT, copr=sample_rFOOT, time=sample_t, com=sample_COM, dcom=sample_dCOM,
                 ddcom=sample_ddCOM, ang=sample_ANG, dang=sample_dANG, q=sample_q, dq=sample_dq, phase=sample_phase)
        plt.show()

    def plot(self, r, q, H, L, dt, rd, qd, omega, Hd, Ld, f1, f2, f3, f4, f5, f6, f7, f8, p1, p2):#, p1, p2, p3, p4, p5, p6, p7, p8):
        dt_plot = np.ones(self.NU)
        dt_plot = np.concatenate(([0.0], dt_plot))
        dt_plot[1:self.N1] *= dt[0]
        dt_plot[self.N1:self.N1+self.N2] *= dt[1]
        ...
        # omega = 2*inv(self.Q(q))@qd
        axs[2][1].plot(dt_plot, omega.T)
        axs[2][1].legend(['omega_x, omega_y, omega_z'])
        ...
        euler = self.euler_from_quaternion(q)
        axs[2][0].plot(dt_plot, euler.T)
        axs[2][0].legend(['angle_x', 'angle_y', 'angle_z'])
