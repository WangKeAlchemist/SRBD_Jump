...
class MotionPlanner:
    def __init__(self, model, T=1, N=50, nlp_solver='ipopt'):
        ...
        # side jump: 1.05
        self.r_init = np.array([0.0, 0.05, 0.58])
        self.r_final = np.array([0.0, 0.05, 0.58])
        self.p_left_init = np.array([0.0, 0.15, 0.0])
        self.p_right_init = np.array([0.0, -0.15, 0.1])
        self.p_left_final = np.array([0.0, 0.15, 0.0])
        self.p_right_final = np.array([0.0, -0.15, 0.1])
        ...
        costs = sumsqr(diff(self.H.T)) + sumsqr(diff(self.L.T)) + 5*sumsqr(self.r) + sumsqr(diff(f1.T)) + sumsqr(diff(f2.T)) + sumsqr(self.rd) \
                + sumsqr(diff(f3.T)) + sumsqr(diff(f4.T)) + sumsqr(diff(f5.T)) + sumsqr(diff(f6.T)) + sumsqr(diff(f7.T)) + sumsqr(diff(f8.T)) \
                + sumsqr(diff(p_left.T)) + sumsqr(diff(p_right.T))
        ...
        for k in range(self.N):
            self.opti.subject_to(sumsqr(self.q[:, k]) == 1)
        for j in range(3):
            self.opti.subject_to(self.dt[j] >= 0.04)
        ...
        for k in range(self.N):
            ...
            if k < self.N1:
                ...
                # single leg support
                self.opti.subject_to(f5[2, k] == np.array([0.0, 0.0, 0.0]))
                self.opti.subject_to(f6[2, k] == np.array([0.0, 0.0, 0.0]))
                self.opti.subject_to(f7[2, k] == np.array([0.0, 0.0, 0.0]))
                self.opti.subject_to(f8[2, k] == np.array([0.0, 0.0, 0.0]))
    ...
    def trajectory_generation(self, r, rd, p_left, p_right, dt, Hd):
        ...
        np.savez('data_single_leg_hopping.npz', copl=sample_lFOOT, copr=sample_rFOOT, time=sample_t, com=sample_COM, dcom=sample_dCOM, ddcom=sample_ddCOM, phase=sample_phase)
        plt.show()
